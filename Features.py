...
║ ──────────────────────────────────────────────────────────────────────────── ║
║                                                                              ║
║   ASSET:    Crash 500 Index           TIMEFRAMES: 8 (5s - 10m)           ║
...
║                                                                              ║
║      [ FEATURE EXTRACTION ONLINE ] v3.0.0-CRASH500 | DERIV WEBSOCKET EDITION          ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
# ============================================================================
# REDIS CLIENT FOR HUGGINGFACE SPACES (CRASH500 — NAMESPACED CHANNELS)
# ============================================================================
try:
    from redis_config_crash500 import REDIS_URL, REDIS_DB_FEATURES, CHANNEL_PREFIX, prefixed_channel
    import redis
    
    class RedisAblyClient:
        """Simple Redis client for HuggingFace Spaces compatibility (CRASH500 namespaced)"""
        ...
        def _connect(self):
            try:
                # CRASH500: Use DB 8 (features) — isolated from v25's DB 0
                self.client = redis.from_url(self.redis_url, db=REDIS_DB_FEATURES)
                self.client.ping()
                print(f"✅ Redis connected for features (CRASH500 — DB {REDIS_DB_FEATURES})")
        ...
        async def publish(self, channel, data):
            if self.client:
                try:
                    # CRASH500: Auto-prefix channel name for namespace isolation
    ...
    class SimpleChannel:
        def __init__(self, name, client):
            self.name = prefixed_channel(name)  # CRASH500: auto-prefix
            self.client = client
    ...
    CHANNEL_PREFIX = "crash500:"
    def prefixed_channel(name):
        return f"crash500:{name}" if not name.startswith("crash500:") else name
    REDIS_DB_FEATURES = 8
...
class DerivBridge:
    ...
    async def _stream_prices(self, deriv_symbol: str):
        """Continuous price streaming for a symbol"""
        try:
            # Subscribe to ticks
            ...
            while self.is_connected:
                try:
                    # THIS IS THE KEY: No timeout, just wait for data
                    data = await self.ws.recv()
                    json_data = json.loads(data)
                    
                    # Check if this is a tick (price) message
                    if 'tick' in json_data:
                        tick_data = json_data['tick']
                        
                        # Only process if it's for our symbol
                        if tick_data.get('symbol') == deriv_symbol:
                            ...
                            async with self._price_lock:
                                ...
                                # Store last tick for symbol_info_tick compatibility
                    ...
                    print(f"❌ WebSocket closed for {deriv_symbol}")
                    self.is_connected = False
                    break
    ...
    async def _ensure_stream(self, deriv_symbol: str):
        """Ensure streaming is active for a symbol"""
        if deriv_symbol not in self._subscribed_symbols:
...
async def main():
    ...
    logger.info("\n📡 Connecting to Redis (CRASH500 namespace)...")
    try:
        ably_client = RedisAblyClient(redis_url=REDIS_URL, use_streams=True)  # CRASH500
        await asyncio.sleep(1)
        logger.info("✅ Redis connected (CRASH500 — channels prefixed with '%s')" % CHANNEL_PREFIX)
    except Exception as e:
        await deriv_bridge.shutdown()
