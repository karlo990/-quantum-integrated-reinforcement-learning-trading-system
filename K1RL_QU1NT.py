...
║ ───────────────────────────────────────────────────────────────────────────  ║
║                                                                              ║
║                   QUANTITATIVE ALPHA GENERATION PLATFORM                     ║
║                                                                              ║
║              Reinforcement Learning • Algorithmic Trading • AI               ║
║                                                                              ║
║ ───────────────────────────────────────────────────────────────────────────  ║
║                                                                              ║
║   STATUS:  ● LIVE        LATENCY: 0.34ms       UPTIME: 99.97%                ║
║   MODELS:  ACTIVE (12)   SIGNALS: 7/min      AUM: $2.4B                    ║
║                                                                              ║
║              "Where Mathematics Meets Market Microstructure"                 ║
║                                                                              ║
║                    [ SYSTEM ONLINE ] v8.5.8 | 2025-11-05                     ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
============================================================================
"""
...
logger.setLevel(logging.DEBUG)  # Change from DEBUG to INFO
...
print("✅ Qiskit logging silenced (WARNING level only)")
# ============================================================================
...
def diagnose_quantum_advisor(system):
    ...
    print("\n" + "="*80)
    print("🔍 QUANTUM ADVISOR DIAGNOSTIC")
    print("="*80)
    ...
    if not hasattr(system, 'quantum_advisor'):
        ...
        if system.quantum_advisor is None:
            ...
            print(f"✅ System has quantum_advisor: {type(system.quantum_advisor).__name__}")
    ...
    print(f"✅ System has {len(system.agents)} agents")
    ...
    if agents_without_advisor:
        ...
        print(f"❌ {len(agents_without_advisor)} agents without quantum_advisor:")
        for agent_name in agents_without_advisor:
            print(f"   - {agent_name}")
    else:
        print(f"✅ All {len(system.agents)} agents have quantum_advisor")
    ...
    if hasattr(system, 'quantum_advisor') and system.quantum_advisor is not None:
        try:
            ...
            if forecast.shape == expected_forecast_shape and confidence.shape == expected_confidence_shape:
                print(f"✅ Quantum advisor prediction test passed")
            ...
            print(f"❌ Quantum advisor prediction test failed: {e}")
    ...
    print("\n" + "="*80)
    if issues:
        print(f"❌ DIAGNOSTIC COMPLETE: {len(issues)} issues found")
...
def fix_quantum_advisor_integration(system):
    ...
    """
    print("\n" + "="*80)
    print("🔧 FIXING QUANTUM ADVISOR INTEGRATION")
    print("="*80)
    ...
    if not hasattr(system, 'quantum_advisor') or system.quantum_advisor is None:
        ...
        try:
            from K1RL_QU1NT import QuantumForecastingAdvisor
            ...
            print(f"   ✅ Created quantum advisor (state_dim={state_dim})")
            fixed.append("Created quantum advisor")
        except Exception as e:
            print(f"   ❌ Failed to create quantum advisor: {e}")
            import traceback
            traceback.print_exc()
            return fixed
    else:
    ...
    if hasattr(system, 'agents') and hasattr(system, 'quantum_advisor'):
        print("\n2️⃣ Linking quantum advisor to agents...")
        linked_count = 0
        ...
        for name, agent in system.agents.items():
            try:
                ...
                linked_count += 1
                print(f"   ✅ [{name}] Linked to quantum advisor")
            except Exception as e:
                print(f"   ❌ [{name}] Failed to link: {e}")
        
        if linked_count > 0:
            fixed.append(f"Linked advisor to {linked_count} agents")
            print(f"\n   ✅ Linked quantum advisor to {linked_count}/{len(system.agents)} agents")
    
    # Fix 3: Enable quantum advisor logging in agents
    print("\n3️⃣ Enabling quantum advisor logging...")
    if hasattr(system, 'agents'):
        for name, agent in system.agents.items():
            # Make sure agent has the quantum advisor attribute
            if not hasattr(agent, 'quantum_advisor'):
                agent.quantum_advisor = system.quantum_advisor
                print(f"   ✅ [{name}] Added quantum_advisor attribute")
            
            # Check if agent's predict method will use quantum advisor
            if hasattr(agent, 'predict'):
                print(f"   ✅ [{name}] Has predict method")
    
    print("\n" + "="*80)
    print(f"✅ FIX COMPLETE: {len(fixed)} fixes applied")
    for fix in fixed:
        print(f"   - {fix}")
    print("="*80 + "\n")
    ...
    try:
        ...
        if tensor is None:
            if RobustConfig.LOG_BOUNDS_VIOLATIONS:
                logging.error(f"❌ {name} is None - creating zero tensor")
            return torch.zeros(1), True
        
        # Check type
        if not isinstance(tensor, torch.Tensor):
            if RobustConfig.LOG_BOUNDS_VIOLATIONS:
                logging.warning(f"⚠️ {name} is not a tensor (type: {type(tensor)})")
            try:
        ...
        if torch.isnan(tensor).any():
            if RobustConfig.LOG_NAN_DETECTION:
                logging.warning(f"⚠️ {name} contains NaN - replacing with zeros")
        ...
        if torch.isinf(tensor).any():
            if RobustConfig.LOG_NAN_DETECTION:
                logging.warning(f"⚠️ {name} contains Inf - replacing with zeros")
        ...
        if magnitude > max_magnitude:
            if RobustConfig.LOG_BOUNDS_VIOLATIONS:
                logging.warning(
                    f"⚠️ {name} magnitude {magnitude:.2f} exceeds limit {max_magnitude:.2f}"
                )
        ...
        logging.error(f"❌ Validation error for {name}: {e}")
    ...
    try:
        ...
        if magnitude < epsilon:
            if RobustConfig.LOG_NORMALIZATION:
                logging.warning(
                    f"⚠️ {name} magnitude too small ({magnitude:.2e}) - returning zeros"
                )
        ...
        if abs(new_magnitude - 1.0) > 0.1:  # Allow 10% tolerance
            if RobustConfig.LOG_NORMALIZATION:
                logging.warning(
                    f"⚠️ {name} normalization check failed: {new_magnitude:.4f} ≠ 1.0"
                )
        
        if RobustConfig.LOG_NORMALIZATION:
            logging.info(f"✓ {name} normalized: {magnitude:.2f} → {new_magnitude:.4f}")
        
        get_health_monitor().record_success(f'{name}_normalization')
        return normalized
        
    except Exception as e:
        logging.error(f"❌ Normalization error for {name}: {e}")
            ...
            try:
                ...
                if fallback_value is not None:
                    if RobustConfig.LOG_FALLBACK_TRIGGERS:
                        logging.warning(f"↩️ {func.__name__} using fallback value")
...
class HealthMonitor:
    ...
    def record_success(self, component: str):
        ...
        if self.errors[component] > 0:
            logging.info(f"✓ {component} recovered after {self.errors[component]} errors")
    ...
    def record_error(self, component: str):
        ...
        if (self.errors[component] >= RobustConfig.ALERT_ON_CONSECUTIVE_ERRORS and
            ...
            logging.critical(
                f"🚨 ALERT: {component} has {self.errors[component]} consecutive errors!"
            )
...
class VotingEnsembleTrainer:
    ...
    **CLASS ARCHITECTURE:**
    ...
    VotingEnsembleTrainer (meta-trainer)
        ├── ConservativeTrainer (voter #1 - safe, small updates)
        ├── AggressiveTrainer  (voter #2 - large, exploratory updates)
        └── AdaptiveTrainer    (voter #3 - dynamically adjusts learning rate)
    ...
    def __init__(
        ...
        logger.critical("✅ VotingEnsembleTrainer initialized with 3 voting strategies")
    ...
    def _create_strategy_optimizers(self):
        ...
        self.adaptive_optimizer = torch.optim.Adam(params, lr=1e-4, betas=(0.9, 0.999))
        ...
        logger.info("🗳️  Created 3 voting strategies:")
    ...
    def _update_strategy_weights(self, loss: float):
        ...
        if self.stats['total_votes'] % 10 == 0:
            logger.info(f"🏆 [VOTING] Best strategy: {best_strategy}")
...
def enable_quantum_advisor_logging(system):
    ...
    """
    print("\n" + "="*80)
    print("📝 ENABLING QUANTUM ADVISOR LOGGING")
    print("="*80)
    ...
    if not logger.handlers:
        ...
        print("✅ Added logging handler")
    
    # Verify quantum advisor will log
    if hasattr(system, 'quantum_advisor') and system.quantum_advisor is not None:
        print("✅ Quantum advisor exists and will log on predictions")
    else:
        print("❌ Quantum advisor missing - logs will not appear")
    ...
    def predict_with_quantum_logging(*args, **kwargs):
        ...
        if hasattr(agent, 'quantum_advisor') and agent.quantum_advisor is not None:
            try:
                ...
                print(f"❌ [{agent_name}] Quantum advisor error: {e}")
        
        return q_values
    
    # Replace the method
    agent.predict = predict_with_quantum_logging
    print(f"✅ Patched predict method for {agent_name}")
...
def patch_all_agents_with_quantum_logging(system):
    ...
    """
    print("\n" + "="*80)
    print("🔧 PATCHING ALL AGENTS WITH QUANTUM ADVISOR LOGGING")
    print("="*80 + "\n")
    
    if not hasattr(system, 'agents'):
        print("❌ System has no agents")
        return
    ...
    print(f"\n✅ Patched {patched_count}/{len(system.agents)} agents")
    print("="*80 + "\n")
...
def apply_complete_quantum_advisor_fix(system):
    ...
    """
    print("\n" + "="*120)
    print(" " * 40 + "🔮 QUANTUM ADVISOR COMPLETE FIX")
    print("="*120 + "\n")
    ...
    if not diagnostic['issues']:
        print(" " * 35 + "✅ QUANTUM ADVISOR FULLY OPERATIONAL")
        print("\n" + " " * 25 + "Predictions will now show quantum advisor influence!")
    else:
        print(" " * 40 + "⚠️ SOME ISSUES REMAIN")
        print("\nRemaining issues:")
...
def patch_qiskit_imports():
    ...
    try:
        ...
        logger.critical("✅ Qiskit available with new API (transpile + run)")
        return True
    except ImportError:
        logger.critical("⚠️ Qiskit not available — using classical fallbacks")
        return False
    except Exception as e:
        logger.critical(f"⚠️ Qiskit import check failed: {e}")
        return False


# -----------------------------------------------------------------------------
# 2️⃣ Quantum Advisor Tensor Dimension Fix
# -----------------------------------------------------------------------------
def fix_quantum_advisor_tensor_dimensions(system):
    ...
    """
    if not hasattr(system, 'quantum_advisor') or system.quantum_advisor is None:
        logger.critical("⚠️ No quantum advisor found in system")
        return False

    advisor = system.quantum_advisor
    if not hasattr(advisor, '_extract_quantum_forecast_features'):
        logger.critical("⚠️ Advisor missing '_extract_quantum_forecast_features'")
        return False

    # The method is already fixed in the class definition
    # Just validate it exists and log success
    logger.critical("✅ Quantum advisor tensor dimension handling validated")
    return True


# -----------------------------------------------------------------------------
# 3️⃣ Apply All Hotfixes (No Fallback Creation)
# -----------------------------------------------------------------------------
def apply_quantum_advisor_hotfixes(system):
    """Apply all quantum advisor hotfixes (without fallback creation)."""
    print("\n" + "="*80)
    print("🔧 APPLYING QUANTUM ADVISOR HOTFIXES (CRITICAL MODE)")
    print("="*80)

    fixes_applied = []

    # Step 1: Patch Qiskit imports
    print("\n1️⃣ Checking Qiskit availability...")
    if patch_qiskit_imports():
        fixes_applied.append("Qiskit available")
        print("   ✅ Qiskit available with new API")
    else:
        print("   ⚠️ Qiskit not available (using classical fallbacks)")

    # Step 2: Fix tensor dimensions
    print("\n2️⃣ Validating quantum advisor tensor dimensions...")
    if fix_quantum_advisor_tensor_dimensions(system):
        fixes_applied.append("Tensor dimensions validated")
        print("   ✅ Tensor dimension handling validated")
    else:
        print("   ⚠️ Could not validate tensor handling (check advisor class)")

    # Step 3: Verify advisor functionality
    print("\n3️⃣ Verifying quantum advisor...")
    try:
        qa = system.quantum_advisor
        ...
        if forecast.shape == expected_forecast_shape and confidence.shape == expected_confidence_shape:
            logger.critical(f"✅ Quantum advisor verified | forecast shape={forecast.shape}, confidence shape={confidence.shape}")
            fixes_applied.append("Quantum advisor verified")
        else:
            logger.critical(f"⚠️ Shape mismatch | forecast={forecast.shape} (expected {expected_forecast_shape}), confidence={confidence.shape} (expected {expected_confidence_shape})")

    except Exception as e:
        logger.critical(f"❌ Advisor verification failed: {e}")
        import traceback
        traceback.print_exc()

    # Summary
    print("\n" + "="*80)
    print(f"✅ HOTFIXES COMPLETE: {len(fixes_applied)} applied")
...
def emergency_fix_qiskit_in_advisor(advisor):
    ...
    try:
        ...
        logger.critical("✅ Emergency fix applied: Disabled Qiskit inside advisor")
        return True

    except Exception as e:
        logger.critical(f"❌ Emergency fix failed: {e}")
        return False
...
logger.info("✅ Qiskit logging silenced (WARNING level only)")
# ============================================================================
...
GRADIENT_CLIP = 1.0
EPSILON_START = 1.0
EPSILON_END = 0.20  # 🔧 CHANGED: Keep 20% exploration (was 1%)
EPSILON_DECAY = 0.9995  # 🔧 CHANGED: Slower decay
...
QUANTUM_WEIGHT = 0.35 # Weight for quantum vs classical
...
def apply_quantum_forecast(system, q_values: Dict[str, torch.Tensor],
    ...
    """
    logger.critical("="*80)
    logger.critical("🔮 QUANTUM FORECAST APPLICATION STARTED")
    logger.critical("="*80)
    ...
    if not hasattr(system, "quantum_advisor") or system.quantum_advisor is None:
        ...
        logger.critical("❌ [QUANTUM FORECAST] Quantum advisor not available - skipping tempering")
        return q_values, metadata
    
    logger.critical(f"✅ [QUANTUM FORECAST] Quantum advisor active | Processing {len(q_values)} agents")
    ...
    try:
        for agent_name, agent_q in q_values.items():
            if agent_q is None:
                ...
                logger.critical(f"⚠️  [QUANTUM FORECAST] {agent_name}: No Q-values to adjust")
                continue
                    ...
                    for tf in timeframe_priority:
                        if tf in agent_states and agent_states[tf] is not None:
                            ...
                            if tf == agent_timeframe:
                                logger.critical(f"📊 [QUANTUM FORECAST] {agent_name}: Using {tf} timeframe state (MATCHED)")
                            else:
                                logger.critical(f"📊 [QUANTUM FORECAST] {agent_name}: Using {tf} timeframe state (FALLBACK)")
                            break
                else:
            ...
            if state_tensor is None:
                ...
                tempering_stats['skipped'] += 1
                logger.critical(f"⚠️  [QUANTUM FORECAST] {agent_name}: No state available - using original Q-values")
                continue
        ...
        logger.critical(f"❌ [QUANTUM FORECAST] ERROR: {e}")
        import traceback
        traceback.print_exc()
...
def quantum_enhanced_predict(system, states_dict: Dict[str, Any]) -> Tuple[Dict[str, torch.Tensor], Dict]:
    ...
    """
    logger.critical("🎯 [PREDICTION] Starting quantum-enhanced prediction")
    ...
    logger.critical(f"📊 [PREDICTION] Base Q-values computed for {len(q_values)} agents")
    ...
    logger.critical(
        f"✅ [PREDICTION] Quantum-enhanced prediction complete | "
...
class QuantumTradingSystem:
    ...
    def verify_system_components(self):
        ...
        if passed == total:
            ...
            print(f"⚠️  {total - passed} component(s) missing or failed")
...
class QuantumStateEncodingWrapperModule(nn.Module):
    ...
    def __init__(self, classical_encoder: nn.Module, n_qubits: int = 20):
        """
        Args:
...
class QuantumForecastingAdvisor(nn.Module):
    ...
    def __init__(
        ...
        print(f"   - Forecast bounds: ±{self.max_forecast_magnitude}")
    ...
    def _extract_quantum_forecast_features(self, state: torch.Tensor) -> torch.Tensor:
        ...
        if torch.isnan(state).any() or torch.isinf(state).any():
            logger.critical("❌ [QUANTUM] State contains NaN/Inf - using fallback")
        elif max_val > 100.0:
            logger.critical(f"❌ [QUANTUM] State magnitude too large: {max_val:.4f} - using fallback")
        elif max_val > 10.0:
            logger.warning(f"⚠️  [QUANTUM] Unusually large state values: {max_val:.4f}")
        
        if state.numel() > 1:
            variance = state.var().item()
            if variance < 1e-8:
                logger.warning(f"⚠️  [QUANTUM] Very low variance: {variance:.2e}")
    ...
    def forward(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if torch.isnan(state).any() or torch.isinf(state).any():
            logger.critical(f"❌ [QUANTUM] State contains NaN/Inf - replacing with zeros")
        ...
        if pre_norm_magnitude > 1000.0:
            logger.critical(
                f"❌ [QUANTUM] Extreme state magnitude: {pre_norm_magnitude:.2f} "
                f"(max element: {pre_norm_max:.2f}) - normalizing"
            )
        elif pre_norm_magnitude > 10.0:
            logger.warning(
                f"⚠️  [QUANTUM] Large state magnitude: {pre_norm_magnitude:.2f} - normalizing"
            )
        ...
        if pre_norm_magnitude > 10.0:
            logger.info(
                f"🔧 [QUANTUM] State normalized: {pre_norm_magnitude:.2f} → "
    ...
    def predict_q_adjustment(
        ...
        with torch.no_grad():
            # Log entry
            logger.critical(f"🔮 [QUANTUM ADVISOR] Processing {state.shape[0]} samples")
            ...
            if positive_ratio > 0.8:
                logger.warning(
                    f"⚠️  [FORECAST BIAS] >80% positive forecasts "
                    f"({positive_ratio*100:.1f}%) - may cause BUY-only behavior"
                )
            elif positive_ratio < 0.2:
                logger.warning(
                    f"⚠️  [FORECAST BIAS] >80% negative forecasts "
            ...
            logger.critical(
                f"📊 [STATE STATS] Mean: {state_mean:.4f}, Std: {state_std:.4f}, "
            ...
            if state_std < 0.1:
                logger.warning(
                    f"⚠️  [STATE VARIANCE] Very low state std ({state_std:.4f}) - "
            ...
            logger.critical(
                f"🔮 [QUANTUM ADVISOR] Forecast range: [{forecast.min().item():.4f}, {forecast.max().item():.4f}] | "
            ...
            if q_values.shape[0] > 0:
                logger.critical(
                    f"🔮 [QUANTUM TEMPERING] Original Q: BUY={q_values[0, 0]:.6f} SELL={q_values[0, 1]:.6f} | "
            ...
            logger.critical(
                f"🔮 [QUANTUM ADVISOR] Tempering complete | "
    ...
    def apply_quantum_tempering_to_qvalues(self, 
        ...
        **KEY CHANGE FROM V8.3**: Forecasts now have ECONOMIC MEANING:
            - Positive forecast (bullish) → BUY favored, SELL disfavored
            - Negative forecast (bearish) → SELL favored, BUY disfavored
        ...
        V8.3 Example (WRONG - symmetric):
            ...
            V8.3 Result: BUY=0.503, SELL=0.403  ❌ Both increase!
        
        V8.4 Example (CORRECT - directional):
            Original: BUY=0.50, SELL=0.40
            Forecast: +0.03 (bullish - price expected to rise)
            V8.4 Result: BUY=0.503, SELL=0.397  ✅ BUY up, SELL down!
        """
        # Type validation
        ...
        if positive_signals == total_signals:
            logger.warning("⚠️  [SIGNAL BIAS] ALL signals positive - will favor BUY across all agents")
        elif positive_signals == 0:
            logger.warning("⚠️  [SIGNAL BIAS] ALL signals negative - will favor SELL across all agents")
        ...
        if signal_before_clip > max_signal:
            logger.warning(
                f"⚠️  [QUANTUM TEMPERING] Signal clipped: {signal_before_clip:.2f} → "
                f"{signal_after_clip:.2f} (max: {max_signal:.2f})"
            )
        
        # STEP 2: Apply DIRECTIONAL adjustment (V8.4)
        # Positive signal → BUY gets +adjustment, SELL gets -adjustment
        # Negative signal → BUY gets -adjustment, SELL gets +adjustment
...
class QuantumSystemTrainer:
    ...
    def _train_on_batch(self, states, actions, rewards, next_states, dones):
        ...
        try:
            ...
            try:
                # CRITIC LOSS: TD error (temporal difference)
                q_loss, ent_loss = self._compute_losses(q_tensors, q_next_tensors, actions, rewards, dones)
                
                # ================================================================
                # ACTOR LOSS: Proper policy gradient for discrete actions
                # ================================================================
                # For discrete action spaces, we use the selected action's Q-value
                # Actor loss = -Q(s, a_selected) where a_selected is the action taken
                # This encourages the policy to select actions with higher Q-values
                # ================================================================
                ...
                for batch_idx, q_dict in enumerate(q_tensors):
                    # Get the action taken for this batch sample
                    # Handle different action tensor formats
                    if isinstance(actions, torch.Tensor):
                        if actions.dim() > 0 and batch_idx < len(actions):
                            action_sample = actions[batch_idx]
                            
                            # Case 1: One-hot encoded [0, 1] or [1, 0]
                            if action_sample.numel() == 2:
                                action_idx = int(torch.argmax(action_sample).item())
                            # Case 2: Single value [0] or [1]
                            elif action_sample.numel() == 1:
                                action_idx = int(action_sample.item())
                            # Case 3: Scalar tensor
                            else:
                                action_idx = int(action_sample.item()) if action_sample.numel() == 1 else 0
                        else:
                            action_idx = 0  # Default to BUY
                    else:
                        action_idx = 0  # Default to BUY
                    
                    # Accumulate Q-values for the action that was taken
                    batch_q_sum = 0.0
                    batch_q_count = 0
                    ...
                    for agent_name, timeframes in q_dict.items():
                        for tf, q_vals in timeframes.items():
                            if isinstance(q_vals, torch.Tensor) and q_vals.numel() > 0:
                                # Get Q-value for the action that was taken
                                if q_vals.dim() > 1 and q_vals.shape[1] >= 2:
                                    # Shape: [batch, actions] - select the action taken
                                    selected_q = q_vals[0, action_idx]
                                elif q_vals.dim() == 1 and len(q_vals) >= 2:
                                    # Shape: [actions] - select the action taken
                                    selected_q = q_vals[action_idx]
                                else:
                                    # Fallback: use mean of available Q-values
                                    selected_q = q_vals.mean()
                                
                                batch_q_sum += selected_q
                                batch_q_count += 1
                    
                    # Add average Q-value for this batch sample (negate to maximize)
                    if batch_q_count > 0:
                        actor_loss += -(batch_q_sum / batch_q_count)
                        num_actor_samples += 1
                
                # Average actor loss across batch
                if num_actor_samples > 0:
                    actor_loss = actor_loss / num_actor_samples
                else:
                    # If we somehow have no samples, use a small constant
                    # This should never happen in practice
                    actor_loss = torch.tensor(0.1, device=self.device)
                    logger.warning("[TRAIN] No actor samples processed! Using constant loss.")
                
                logger.debug(f"[LOSS] Actor: {actor_loss.item():.4f} (from {num_actor_samples} samples)")
                logger.debug(f"[LOSS] Critic: {q_loss.item():.4f}")
            ...
            # ✅ CRITICAL FIX: Combine actor loss, critic loss, and entanglement
            # Actor loss: Maximize Q-values (policy improvement)
            # Critic loss: Minimize TD error (value estimation)
            # Entanglement loss: Decorrelate agents
            total_loss = actor_loss + q_loss + 0.01 * ent_loss
            ...
            for agent_name in self.system.agent_names:
                ...
                if hasattr(agent, "actor_optimizer") and hasattr(agent, "actor"):
                    try:
                        ...
                        agent.actor_optimizer.step()
                        actor_losses.append(actor_loss.item())  # ✅ Use actor_loss
                        actors_updated += 1
            ...
            # 🔧 CRITICAL PATCH: Ensure losses are never empty
            if not actor_losses:
                logger.critical("[TRAIN] ⚠️  No actors were updated! Using actor_loss as fallback.")
...
class QuantumAgent(nn.Module):
    ...
    def get_action(self, state: torch.Tensor, epsilon: Optional[float] = None) -> int:
        ...
        # 🔧 NEW: Force minimum exploration even when epsilon is low
        effective_epsilon = max(epsilon, 0.15)  # Never drop below 15% random
...
class QuantumSystemBridge:
    ...
    def __init__(self, agent_names: List[str], state_dim: int = 58,
        ...
        try:
            if self.quantum_system is not None:
                ...
                logger.critical("✅ Base quantum trainer initialized successfully")
                
                # =====================================================================
                # V8.5: WRAP WITH VOTING ENSEMBLE TRAINER
                # =====================================================================
                logger.critical("🗳️  V8.5: Wrapping trainer with VotingEnsembleTrainer...")
                logger.critical("   • Adds 3 training strategies that vote on gradient updates")
                logger.critical("   • Conservative (LR=5e-5), Aggressive (LR=5e-4), Adaptive (LR=1e-4)")
                logger.critical("   • Only applies updates when strategies reach consensus (60%)")
                logger.critical("   • Your QuantumVotingSystem (action voting) remains unchanged")
                ...
                logger.critical("✅ V8.5: Voting Ensemble Trainer activated!")
                logger.critical("   • Training now uses DUAL VOTING SYSTEMS:")
                logger.critical("   • Level 1: Gradient voting (VotingEnsembleTrainer) - NEW")
                logger.critical("   • Level 2: Action voting (QuantumVotingSystem) - UNCHANGED")
                
                # ================================================================
                # V8.5.8 FIX: EXPLICIT BUFFER VERIFICATION AND LINKING
                # ================================================================
                logger.critical("")
                logger.critical("🔧 V8.5.8: Verifying trainer buffer linkage...")
                ...
                try:
                    ...
                    logger.critical(f"   ✅ Buffer property accessible: {type(test_buffer).__name__ if test_buffer else 'None'}")
                    
                    if test_buffer is None:
                        logger.warning("   ⚠️  Buffer property returns None, setting explicitly...")
                        self.quantum_trainer.buffer = self.hybrid_buffer
                        logger.critical("   ✅ Buffer set via property setter")
                    elif test_buffer is not self.hybrid_buffer:
                        logger.warning("   ⚠️  Buffer is different object, relinking...")
                        self.quantum_trainer.buffer = self.hybrid_buffer
                        logger.critical("   ✅ Buffer relinked to hybrid_buffer")
                    else:
                        logger.critical("   ✅ Buffer already correctly linked!")
                        
                except AttributeError as e:
                    logger.error(f"   ❌ Buffer property not accessible: {e}")
                    logger.critical("   🔧 Setting buffer directly on base_trainer...")
                    if hasattr(self.quantum_trainer, 'base_trainer'):
                        self.quantum_trainer.base_trainer.buffer = self.hybrid_buffer
                        logger.critical("   ✅ Buffer set on base_trainer")
                    buffer_property_works = False
                except Exception as e:
                ...
                try:
                    ...
                    if final_buffer is self.hybrid_buffer:
                        logger.critical("   ✅ FINAL CHECK: Trainer buffer correctly linked!")
                    elif final_buffer is None:
                        logger.error("   ❌ FINAL CHECK: Buffer is None!")
                    else:
                        logger.warning(f"   ⚠️  FINAL CHECK: Buffer objects don't match")
    ...
    def predict_single_agent(self, agent_name: str, states_dict: Optional[dict] = None) -> np.ndarray:
        """
        Enhanced predict_single_agent with quantum advisor integration and CRITICAL logging.
        Returns a safe 1D numpy array of quantum-advised q-values/probabilities for a single agent.
        """
        logger.critical(f"🎯 [SINGLE AGENT] Predicting for agent: {agent_name}")
        ...
        try:
            ...
            if metadata.get('quantum_forecast', {}).get('applied', False):
                logger.critical(f"✅ [SINGLE AGENT] {agent_name}: Using quantum-adjusted Q-values")
            else:
                logger.critical(f"⚠️  [SINGLE AGENT] {agent_name}: Using base Q-values (no quantum adjustment)")
            ...
            if q_vals.size >= 2:
                logger.critical(
                    f"📊 [SINGLE AGENT] {agent_name}: Q-values: BUY={q_vals[0]:.6f} SELL={q_vals[1]:.6f} | "
                    f"Action: {'BUY' if q_vals[0] > q_vals[1] else 'SELL'}"
                )
            ...
            logger.critical(f"❌ [SINGLE AGENT] {agent_name}: Prediction error: {e}")
            import traceback
            traceback.print_exc()
    ...
    def predict_all_agents_with_metadata(self) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        """Return safe q-values dict (1D arrays) for all agents with quantum advisor tempering.
        
        V8.1 FIX: Call base predict, then apply quantum forecast separately to avoid recursion.
        """
            ...
            if hasattr(self.quantum_bridge, 'quantum_trainer'):
                if self.quantum_bridge.quantum_trainer:
                    ...
                    try:
                        ...
                        logger.warning(f"      ⚠️  Unexpected error accessing buffer: {e}")
                        buffer_accessible = False
                            ...
                            if not is_linked and self.quantum_bridge.hybrid_buffer:
                                ...
                                try:
                                    ...
                                    if hasattr(trainer, 'base_trainer'):
                                        ...
                                        logger.critical("      ✅ Buffers linked via base_trainer")
                        else:
                            logger.critical("      ❌ Trainer Buffer: is None")
                            # Try to set it
                            logger.critical("      🔧 Attempting to set buffer...")
                            try:
                                trainer.buffer = self.quantum_bridge.hybrid_buffer
                                logger.critical("      ✅ Buffer set successfully")
                            except Exception as e:
                                logger.error(f"      ❌ Failed to set buffer: {e}")
                    else:
                        logger.critical("      ❌ Trainer Buffer: not accessible (no property or attribute)")
                        # Check if it's a wrapper with base_trainer
                        if hasattr(trainer, 'base_trainer'):
                            logger.critical("      🔧 Detected wrapper trainer, checking base_trainer...")
                            if hasattr(trainer.base_trainer, 'buffer'):
                                base_buffer = trainer.base_trainer.buffer
                                if base_buffer:
                                    logger.critical(f"      ✅ base_trainer.buffer exists: {len(base_buffer)} experiences")
                                else:
                                    logger.critical("      ⚠️  base_trainer.buffer is None")
                            else:
                                logger.critical("      ❌ base_trainer has no buffer attribute")
                else:
    ...
    async def on_feature_message(self, message):
        ...
        try:
            ...
            if name == 'xs':  # Limit output to one agent to prevent spam
                logger.info("=" * 80)
                logger.info(f"📊 FEATURE MESSAGE DIAGNOSTIC - Agent: {name}")
                logger.info("=" * 80)
...
def ensure_quantum_advisor_accessible_v82(system, quantum_advisor):
    ...
    """
    logger.critical("="*80)
    logger.critical("🔧 V8.2 FIX 1/4: QUANTUM ADVISOR ACCESSIBILITY")
    logger.critical("="*80)
    
    # 1. Add to quantum_system
    if hasattr(system, 'quantum_system'):
        system.quantum_system.quantum_advisor = quantum_advisor
        logger.critical("✅ Added to system.quantum_system")
    
    # 2. Add to system itself
    system.quantum_advisor = quantum_advisor
    logger.critical("✅ Added to system")
    
    # 3. Add to all agents
    if hasattr(system, 'agents'):
        for name, agent in system.agents.items():
            agent.quantum_advisor = quantum_advisor
        logger.critical(f"✅ Added to {len(system.agents)} agents")
...
def patch_prediction_method_v82(system):
    ...
    """
    logger.critical("="*80)
    logger.critical("🔧 V8.2 FIX 2/4: PREDICTION METHOD PATCH")
    logger.critical("="*80)
    
    if not hasattr(system, 'predict_all_agents_with_metadata'):
        logger.critical("❌ No predict_all_agents_with_metadata method found")
        return
    ...
    def forced_quantum_predict_v82():
        ...
        try:
            ...
            logger.critical(f"❌ Original predict failed: {e}")
            return {}, {}
        ...
        if quantum_advisor is not None:
            logger.critical("🔮 FORCING QUANTUM ADVISOR APPLICATION (V8.2)")
            ...
            if states_dict:
                ...
                for agent_name, q in q_values.items():
                    try:
                        ...
                        logger.critical(
                            f"✅ {agent_name}: BUY: {q[0]:.4f}→{adjusted_q[agent_name][0]:.4f} | "
                            f"SELL: {q[1]:.4f}→{adjusted_q[agent_name][1]:.4f}"
                        )
                    
                    except Exception as e:
                        logger.critical(f"❌ {agent_name}: Quantum adjustment failed: {e}")
                        adjusted_q[agent_name] = q
    ...
    logger.critical("✅ Patched prediction method to force quantum advisor (V8.2)")
    logger.critical("="*80)
...
def enable_training_logging_v82(system):
    ...
    """
    logger.critical("="*80)
    logger.critical("🔧 V8.2 FIX 3/4: TRAINING ACTIVATION")
    logger.critical("="*80)
    ...
    if hasattr(system, 'quantum_trainer'):
        ...
        logger.critical("✓ Found trainer at: system.quantum_trainer")
        ...
        if hasattr(system.quantum_bridge, 'quantum_trainer'):
            ...
            logger.critical("✓ Found trainer at: system.quantum_bridge.quantum_trainer")
        elif hasattr(system.quantum_bridge, 'trainer'):
            trainer = system.quantum_bridge.trainer
            logger.critical("✓ Found trainer at: system.quantum_bridge.trainer")
    
    if trainer is None:
        logger.critical("❌ No trainer found")
    ...
    if hasattr(trainer, 'train_step'):
        ...
        def logged_train_step_v82(*args, **kwargs):
            logger.critical("🎓 TRAINING STEP EXECUTING (V8.2)...")
            try:
                result = original_train(*args, **kwargs)
                logger.critical(f"✅ TRAINING COMPLETE (V8.2) | Result: {result}")
                return result
            except Exception as e:
                logger.critical(f"❌ TRAINING FAILED (V8.2): {e}")
                import traceback
                traceback.print_exc()
                raise
        
        trainer.train_step = logged_train_step_v82
        logger.critical(f"✅ Added logging to train_step (V8.2) | Trainer type: {type(trainer).__name__}")
    ...
    if hasattr(system, 'reward_batch_processor'):
        ...
        if hasattr(processor, 'running') and not processor.running:
            logger.critical("⚠️  Starting RewardBatchProcessor (V8.2)...")
            try:
                if hasattr(processor, 'start'):
                    processor.start()
                    logger.critical("✅ Started RewardBatchProcessor (V8.2)")
            except Exception as e:
                logger.critical(f"❌ Failed to start: {e}")
        else:
            logger.critical("✅ RewardBatchProcessor already running")
        ...
        if hasattr(processor, 'process_batch'):
            ...
            def logged_process_batch_v82(batch):
                logger.critical(f"🔄 PROCESSING BATCH (V8.2) | Size: {len(batch)}")
                result = original_process(batch)
                logger.critical(f"✅ BATCH PROCESSED (V8.2)")
                return result
            
            processor.process_batch = logged_process_batch_v82
            logger.critical("✅ Added logging to batch processor (V8.2)")
...
def link_buffer_trainer_v82(system):
    ...
    """
    logger.critical("="*80)
    logger.critical("🔧 V8.2 FIX 4/4: BUFFER-TRAINER LINKAGE")
    logger.critical("="*80)
    ...
    if hasattr(system, 'hybrid_buffer'):
        ...
        logger.critical("✓ Found buffer at: system.hybrid_buffer")
        ...
        logger.critical("✓ Found buffer at: system.quantum_bridge.hybrid_buffer")
    
    if buffer is None:
        logger.critical("❌ No buffer found")
        return
    ...
    logger.critical(f"✅ Trainer found: {type(trainer).__name__}")
    
    # Link them - V8.5.8 FIX: Handle @property buffer correctly
    buffer_exists = False
    current_buffer = None
    ...
    try:
        ...
        buffer_exists = True
        logger.critical(f"✓ Trainer has buffer: {type(current_buffer).__name__ if current_buffer else 'None'}")
    except AttributeError:
        logger.warning("⚠️  Trainer has no buffer attribute/property")
        buffer_exists = False
    except Exception as e:
        logger.warning(f"⚠️  Error accessing trainer.buffer: {e}")
        buffer_exists = False
    ...
    if buffer_exists:
        if current_buffer is not buffer:
            try:
                ...
                logger.critical("✅ Linked trainer to buffer (V8.2)")
            except Exception as e:
                logger.error(f"❌ Failed to set buffer: {e}")
                # Try setting on base_trainer if it's a wrapper
                if hasattr(trainer, 'base_trainer'):
                    trainer.base_trainer.buffer = buffer
                    logger.critical("✅ Linked base_trainer to buffer (V8.2)")
        else:
            logger.critical("✅ Already linked")
    else:
        # Try to create the attribute
        try:
            trainer.buffer = buffer
            logger.critical("✅ Created buffer link (V8.2)")
        except Exception as e:
            logger.error(f"❌ Cannot create buffer attribute: {e}")
            # Try setting on base_trainer if it's a wrapper
            if hasattr(trainer, 'base_trainer'):
                trainer.base_trainer.buffer = buffer
                logger.critical("✅ Created buffer link on base_trainer (V8.2)")
    ...
    if hasattr(buffer, 'add'):
        ...
        def logged_add_v82(*args, **kwargs):
            ...
            if counter['count'] % 10 == 0:
                size = buffer.size() if hasattr(buffer, 'size') else len(buffer)
                logger.critical(f"📦 BUFFER SIZE (V8.2): {size}")
            return result
        
        buffer.add = logged_add_v82
        logger.critical("✅ Added buffer logging (V8.2)")
...
def apply_all_critical_fixes_v82(system, quantum_advisor):
    ...
    try:
        ...
        logger.critical("")
        logger.critical("#"*80)
        logger.critical("# ✅ ALL V8.2 CRITICAL FIXES APPLIED SUCCESSFULLY")
        logger.critical("#"*80)
        logger.critical("")
        logger.critical("Your system will now:")
        logger.critical("  ✅ Apply quantum advisor to every prediction")
        logger.critical("  ✅ Log all training steps")
        logger.critical("  ✅ Store experiences properly")
        logger.critical("  ✅ Show all operations in logs")
        logger.critical("")
        logger.critical("Watch for these logs:")
        logger.critical("  🔮 FORCING QUANTUM ADVISOR APPLICATION (V8.2)")
        logger.critical("  ✅ Agent quantum adjustments: BUY: X→Y")
        logger.critical("  📦 BUFFER SIZE (V8.2): N")
        logger.critical("  🎓 TRAINING STEP EXECUTING (V8.2)")
        logger.critical("  ✅ TRAINING COMPLETE (V8.2)")
        logger.critical("")
        logger.critical("If you see all of these → YOUR SYSTEM IS LEARNING! 🎉")
        logger.critical("")
        logger.critical("#"*80)
        logger.critical("")
        ...
        logger.critical("")
        logger.critical("#"*80)
        logger.critical(f"# ❌ V8.2 FIX INTEGRATION FAILED: {e}")
        logger.critical("#"*80)
        logger.critical("")
        import traceback
        traceback.print_exc()
...
def verify_v82_fixes(system):
    ...
    logger.critical("")
    logger.critical("="*80)
    logger.critical("🔍 VERIFYING V8.2 FIXES")
    logger.critical("="*80)
    ...
    total += 1
    if hasattr(system, 'quantum_advisor') and system.quantum_advisor is not None:
        checks.append("✅ Quantum advisor on system")
        passed += 1
    else:
        checks.append("❌ No quantum advisor on system")
    
    # Check 2: Prediction patched
    total += 1
    if hasattr(system, '_original_predict_all_agents_v82'):
        checks.append("✅ Prediction method patched (V8.2)")
        passed += 1
    else:
        checks.append("❌ Prediction method not patched")
    ...
    if hasattr(system, 'hybrid_buffer') or (hasattr(system, 'quantum_bridge') and hasattr(system.quantum_bridge, 'hybrid_buffer')):
        checks.append("✅ Buffer found")
        passed += 1
    else:
        checks.append("❌ Buffer not found")
    
    # Check 4: Trainer exists - V8.5.8 FIX: Check quantum_trainer
    total += 1
    trainer_found = False
    if hasattr(system, 'quantum_trainer'):
        checks.append("✅ Trainer found at system.quantum_trainer")
        trainer_found = True
    elif hasattr(system, 'quantum_bridge'):
        if hasattr(system.quantum_bridge, 'quantum_trainer'):
            checks.append("✅ Trainer found at system.quantum_bridge.quantum_trainer")
            trainer_found = True
        elif hasattr(system.quantum_bridge, 'trainer'):
            checks.append("✅ Trainer found at system.quantum_bridge.trainer")
            trainer_found = True
    
    if trainer_found:
        passed += 1
    else:
        checks.append("❌ Trainer not found")
    ...
    if passed == total:
        logger.critical("="*80)
        logger.critical("✅ V8.2 VERIFICATION PASSED")
        logger.critical("="*80)
        ...
        logger.critical("="*80)
        logger.critical("⚠️  V8.2 VERIFICATION INCOMPLETE")
        logger.critical("="*80)
...
def verify_quantum_advisor_integration(system):
    ...
    """
    logger.critical("="*80)
    logger.critical("🔍 QUANTUM ADVISOR INTEGRATION VERIFICATION (V7)")
    logger.critical("="*80)
    ...
    try:
        ...
        if hasattr(system, 'quantum_advisor') and system.quantum_advisor is not None:
            ...
            logger.critical("✅ System has quantum_advisor attribute")
            ...
            if hasattr(system.quantum_advisor, 'predict_q_adjustment'):
                ...
                logger.critical("✅ quantum_advisor.predict_q_adjustment() method exists")
                ...
                try:
                    ...
                    logger.critical(f"✅ predict_q_adjustment() test passed | Adjustment: {(adjusted_q - test_q).abs().mean().item():.6f}")
                except Exception as e:
                    logger.critical(f"❌ predict_q_adjustment() test failed: {e}")
            else:
                logger.critical("❌ predict_q_adjustment() method MISSING!")
        else:
            logger.critical("❌ quantum_advisor NOT FOUND on system")
        ...
        if hasattr(system, 'quantum_system') and system.quantum_system is not None:
            if hasattr(system.quantum_system, 'quantum_advisor'):
                ...
                logger.critical("✅ quantum_system has quantum_advisor")
        ...
        if hasattr(system, 'quantum_bridge'):
            try:
                ...
                logger.critical(f"✅ quantum_bridge.predict_single_agent() works | Q-values: {test_result}")
            except Exception as e:
                logger.critical(f"❌ quantum_bridge.predict_single_agent() failed: {e}")
        
        # Check 5: QUANTUM_WEIGHT configured
        try:
            weight = QUANTUM_WEIGHT
            checks['quantum_weight_configured'] = True
            logger.critical(f"✅ QUANTUM_WEIGHT = {weight}")
        except:
            logger.critical("❌ QUANTUM_WEIGHT not defined")
        
    except Exception as e:
        logger.critical(f"❌ Verification error: {e}")
    
    # Summary
    logger.critical("="*80)
    passed = sum(checks.values())
    total = len(checks)
    logger.critical(f"📊 VERIFICATION RESULTS: {passed}/{total} checks passed")
    
    if passed == total:
        logger.critical("✅✅✅ ALL CHECKS PASSED - Quantum advisor fully integrated! ✅✅✅")
    else:
        logger.critical("❌ SOME CHECKS FAILED - Review issues above")
        for check, status in checks.items():
            if not status:
                logger.critical(f"   ❌ {check}")
...
def quantum_integrated_main():
    ...
    # - "❌ NO QUANTUM ADVISOR FOUND" in predictions
    # - No training happening
    # - Identical Q-values forever
    # ============================================================================
    logger.critical("="*80)
    logger.critical("🔥 APPLYING V8.2 CRITICAL FIXES")
    logger.critical("="*80)
    logger.critical("")
    ...
    try:
        ...
        if hasattr(system, 'quantum_advisor') and system.quantum_advisor is not None:
            ...
            logger.critical(f"✓ Found quantum_advisor on system: {type(quantum_advisor).__name__}")
            ...
            if hasattr(system.quantum_bridge.quantum_system, 'quantum_advisor'):
                ...
                logger.critical(f"✓ Found quantum_advisor on quantum_system: {type(quantum_advisor).__name__}")
        
        # Create quantum advisor if somehow still missing
        if quantum_advisor is None:
            logger.critical("⚠️  WARNING: quantum_advisor not found, creating it now...")
            ...
            system.quantum_advisor = quantum_advisor
            logger.critical("✓ Created new quantum_advisor")
        ...
        if success:
            logger.critical("")
            logger.critical("="*80)
            logger.critical("✅ V8.2 FIXES APPLIED SUCCESSFULLY")
            logger.critical("="*80)
            logger.critical("")
            ...
            logger.critical("")
            logger.critical("🎉 SYSTEM IS NOW READY TO LEARN!")
            logger.critical("")
            logger.critical("Watch for these logs during trading:")
            logger.critical("   🔮 FORCING QUANTUM ADVISOR APPLICATION (V8.2)")
            logger.critical("   ✅ xs: BUY: 0.0427→0.0501 | SELL: 0.0843→0.0921")
            logger.critical("   📦 BUFFER SIZE (V8.2): 70")
            logger.critical("   🎓 TRAINING STEP EXECUTING (V8.2)...")
            logger.critical("   ✅ TRAINING COMPLETE (V8.2) | losses shown")
            logger.critical("")
            logger.critical("If you see ALL of these → Your system is learning! 🎉")

            logger.critical("="*80)
            logger.critical("🔍 V8.5.2 COMPONENT VERIFICATION")
            logger.critical("="*80)
            logger.critical("")
            ...
            try:
                ...
                logger.critical(f"⚠️  Component verification error: {e}")
            ...
            logger.critical("")
            logger.critical("="*80)
            logger.critical("❌ V8.2 FIX APPLICATION FAILED")
            logger.critical("="*80)
        ...
        logger.critical("")
        logger.critical("="*80)
        logger.critical(f"❌ V8.2 FIX ERROR: {e}")
        logger.critical("="*80)
        import traceback
        traceback.print_exc()
        logger.critical("")
        logger.critical("System will continue but V8.2 fixes may not be active")
        logger.critical("You may see '❌ NO QUANTUM ADVISOR FOUND' in predictions")
        logger.critical("")
    ...
    # ============================================================================
    print("\n" + "="*80)
    print("🎓 TRAINING LOOP ACTIVE")
    print("="*80)
    print("   • Training every 10 seconds when buffer >= 64")
    print("   • Batch size: 32")
    print("   • Only training output will be shown below")
    print("="*80 + "\n")
                ...
                try:
                    ...
                    if hasattr(system, 'quantum_bridge'):
                        ...
                        if hasattr(system.quantum_bridge, 'hybrid_buffer'):
                            buffer = system.quantum_bridge.hybrid_buffer
                            buffer_size = buffer.size() if hasattr(buffer, 'size') else len(buffer) if buffer else 0
                    ...
                    if trainer and buffer and buffer_size >= 64:
                        ...
                        # ================================================================
                        # PRINT TO CONSOLE - TRAINING START
                        # ================================================================
                        print("\n" + "🎓"*40)
                        print(f"TRAINING #{training_counter} STARTING...")
                        print(f"Buffer: {buffer_size} experiences | Batch: 32")
                        print("🎓"*40)
                        # ================================================================
                        
                        # Logger silenced (only print statements shown)
                        
                        try:
                            # Call training step
                            train_stats = trainer.train_step(batch_size=32)
                            
                            # ================================================================
                            # PRINT TRAINING RESULTS TO CONSOLE (VISIBLE OUTPUT)
                            # ================================================================
                            print("\n" + "="*80)
                            print(f"🎓 TRAINING #{training_counter} COMPLETE")
                            print("="*80)
                            
                            if train_stats:
                                # Extract losses
                                actor_loss = train_stats.get('actor_loss', train_stats.get('total_loss', 'N/A'))
                                critic_loss = train_stats.get('critic_loss', 'N/A')
                                ...
                                print(f"📊 All metrics: {train_stats}")
                            else:
                                print("⚠️  No training stats returned (train_stats is None)")
                            ...
                            # ================================================================
                            print("\n" + "❌"*40)
                            print(f"TRAINING #{training_counter} FAILED!")
                            print(f"Error: {train_error}")
                            print("❌"*40 + "\n")
                            # ================================================================
                            
                            # Logger silenced (only print statements and traceback shown)
                            import traceback
                            traceback.print_exc()
                    
                    elif not trainer:
                        if training_counter == 0:  # Only log once at startup
                            logger.warning("⚠️  [TRAINING] Trainer not found - checking locations...")
                            # Diagnostic logging
                        ...
                        if training_counter % 6 == 0:  # Log every minute (6 * 10s)
                            logger.info(
                                f"📊 [TRAINING] Waiting for buffer | "
                                f"Current: {buffer_size}/64 required"
                            )
                    
                except Exception as e:
                    logger.error(f"❌ [TRAINING LOOP] Unexpected error: {e}")
                    import traceback
                    traceback.print_exc()
...
def main():
    ...
    print("\n" + "="*80)
    print("🔇 ALL LOGGING DISABLED - ONLY TRAINING PRINTS WILL SHOW")
    print("="*80)
    print()
    # ============================================================================
    
    print("\n" + "="*80)
    print("QUANTUM TRADING SYSTEM V8.5.8 - TRAINING ACTIVATED")
    print("="*80)
    print("   - Pure quantum predictions (no classical fallback)")
    print("   - Multi-timeframe entanglement")
    print("   - Quantum voting system")
    print("   - ✨ QUANTUM ADVISOR Q-VALUE TEMPERING (ACTIVE)")
    print("   - 🔮 Complete CRITICAL-level logging")
    print("   - ⚡ V8.5.5: Each agent uses its OWN timeframe state")
    print("   - 🎯 V8.5.6: Forecast bias detection and diagnostics")
    print("   - 📊 V8.5.6: State variance and signal monitoring")
    print("   - 🔇 V8.5.7: Qiskit verbose logging silenced")
    print("   - 🎓 V8.5.8: TRAINING LOOP ACTIVE (every 10s)")
    print("   - Automatic periodic resets (3000 minutes)")
    print("   - All helper functions maintained")
    print("="*80 + "\n")
    
    print("⏳ Initializing system (this may take 30-60 seconds)...\n")
    ...
    print("✅ System initialized! Training will begin when buffer >= 64 experiences.\n")
    print("="*80)
